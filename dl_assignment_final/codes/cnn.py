...
def main(_):
    ...
    with tf.variable_scope('network'):
        ...
        with tf.variable_scope('process'):
            ...
            hidden1 = conv(hidden, 2, tf.nn.relu, initializer, regularizer, 2)
            hidden2 = conv(hidden, 3, tf.nn.relu, initializer, regularizer, 2)
            hidden3 = conv(hidden, 4, tf.nn.relu, initializer, regularizer, 2)
            ...
            hidden = tf.layers.dropout(hidden, rate=0.5, training=training)
    ...
    plot(((plt_X, plt_train), (plt_X, plt_eval)),
         legends=('Train', 'Eval'), log='cnn-loss.log', fig='cnn-loss.png')
    plot(((plt_X, plt_AUC),),
         legends=('Eval AUC',), log='cnn-AUC.log', fig='cnn-AUC.png')
    plot(((plt_X, plt_AP),),
         legends=('Eval AP',), log='cnn-AP.log', fig='cnn-AP.png')
    ...
    print('test: loss: {} auc: {} ap: {}'
          .format((eval_loss, eval_cls_loss, eval_reg_loss),
                  auc, ap))
    with open('cnn.predicts', 'wb') as writer:
